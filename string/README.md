...
- - RK算法：利用滚动hash做匹配；滚动hash利用上一次的hash计算本次hash，o(1),整体 为n
  - KMP算法：使用匹配表减少原字符串的回溯次数；部分匹配表，记录下标 j 之前（包括 j）的模式串 p 中，最长相等前后缀的长度；，n+m
  - - 最长相等前缀后缀算法:next[j]标识p[0:j]中最长相等前缀后缀的长度；
    - 双指针，left为前缀结束位置，同时也标识前缀长度，可以回退；right为后缀结束位置
    - 回退到next[i-1]: 即p[0:i-1]的最长相等前后缀为next[i-1]
```
t[i:m+1] 与 p[0:m+1]匹配失败 =》 t[i:m] == p[0:m]
next[m] = k => p[0:k-1] == p[m-k+1:m] == t[m-k+1:m]
由以上可得出 =》 p[0:k-1] == t[m-k+1:m]   得出下一次比较从p[k]开始和t[m+1]比较

对于next求解：AABAAAB
已知：
p[j+1] != p[i+1] => p[0:j] == p[i-j-1:i]
next[j] = k => p[0:k-1] == p[j-k+1:j] = p[i-k+1:i]
  得出下一次比较可以从p[k]开始和p[i+1]比较
```
- - BM算法：先对模式串p 进行预处理，生成启发规则表，选择后移位数最多的
  - - 坏字符规则： 在位置j不匹配的字符c，将p移动 j-last_c_in_p个位置
    - 好后缀规则（从右向左比较）： 好后缀=已经匹配的后缀；3种方案：1.移动到p中前一个匹配后缀的子串位置；2.p的前缀匹配后后缀；3.移动整个p
    - - suffix[i]=s 表示为以下标 i 为结尾的子串与模式串后缀匹配的最大长度为 s
- - Horspool算法：BM的简化，仅使用坏字符规则，条件是匹配的最后一个字符必须出现在原字符中，否则移动整个字符串；o(n) -> o（n*m）
  - - 移动从左到右，比较从右到左
- - Sunday算法：类似BM，比较是从左到右
- 子串相关问题。
- 前缀 / 后缀相关问题；
- 回文串相关问题。
- 子序列相关问题。
