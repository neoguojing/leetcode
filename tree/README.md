...
- 处理不交集的查询和合并问题
- - 将两个集合合并成一个集合
  - 确定某个元素属于哪个集合。通常是返回集合内的一个「代表元素」
- 合并 union(x, y)：将集合x 和集合 y 合并成一个集合。
- 查找 find(x)：查找元素 x 属于哪个集合。
- 查找 is_connected(x, y)：查询元素 x 和 y 是否在同一个集合中。
### 树状数组
- 数列的前缀和，区间和。它可以以 O(logn) 的时间得到任意前缀
### 线段树 用于在区间信息上进行统计 log2n
...
- no 226 二叉树反转：解法：等价于将左右子树交换，然后再分别反转左子树和右子树
### 二叉树搜索树：
...
- 236 普通二叉搜索树的的LCA问题：递归：使用后序遍历，查找p，q的节点；左子树和右子树都找到节点，则公共祖先是根；左子树和右子树都未找到节点，则返回nil；左子树找到，右子树未找到，则返回左节点；否则返回右节点。
### trie 前缀树
- 利用字符串的公共前缀来降低查询时间的开销， 时间n ，空间d^n
...
- > 1.最大路径经过root，在左子树，在右子树，三种情况，可以归结为比较三种情况的大小，使用递归；
- > 2.对于小于0的节点，无需计算
