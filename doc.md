...
- > 1. j>=coin[i-1]；dp[i][j] = dp[i-1][j] +dp[i][j-coins[i-1]] ;; 前i-1个硬币构成j的个数+前i个硬币构成j-coins[i-1]的个数
- > 2.j<coin[i-1];dp[i][j] = dp[i-1][j]; 前i个硬币构成j的个数等于前i-1个硬币构成j的个数
- > 总结：类似于dp[i][j] = dp[i-1][j]+dp[i][j-1],此处j的取值不是连续的
```
示例：
1和2，构成5；若第3个硬币大于5，则不需要考虑其他情况，条件2
1和2，构成5；第三个硬币为5，1，2和5构成5-2 =3 的个数；相加，条件1
```                        
